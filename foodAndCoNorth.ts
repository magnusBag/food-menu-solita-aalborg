...
export async function foodAndCoNorth(): Promise<Response> {
  try {
    const date = new Date();
    ...
    const url =
      `https://www.shop.foodandco.dk/api/WeeklyMenu?restaurantId=1073&languageCode=da-DK&date=${formattedDate}`;
    ...
    return new Response("Error occurred while scraping the menu.", {
      status: 500,
    });
  }
}
