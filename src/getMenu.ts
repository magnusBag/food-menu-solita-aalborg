...
export const getMenu = async () => {
    ...
    const res = await fetch("https://app.kanpla.dk/api/internal/load/frontend", {
      ...
      const meatDishes = Object.values(meatDates).filter((x: any) => x.menu).map((x: any) => ({...x.menu, type: "meat" }));
      const veggieDishes = Object.values(veggieDates).filter((x: any) => x.menu).map((x: any) => ({...x.menu, type: "veggie" }));
...
const getToken = async () => {
  const res = await fetch("https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDDtuovWpK6PARKIt9wUqaTQP7MjFWIWF4", {
       "headers": {
         "accept": "*/*",
         "accept-language": "en,da;q=0.9,en-US;q=0.8",
         "content-type": "application/json",
         ...
         "x-client-data": "CIW2yQEIpLbJAQipncoBCN/3ygEIkqHLAQj+ossBCJv+zAEIhqDNAQjJ4c4BGI/OzQE=",
